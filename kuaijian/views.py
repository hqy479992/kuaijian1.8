...
from global_var import get_val
...
def settingsvalue(request):
	...
	try:
		...
		if len(audio_list) > 1:
			audio_path = ExtractAudioTrack(task_name, audio_list)
		else:
		...
		cc = ClipControler(task_name, video_list, audio_path, "./static/resultfiles/" + task_name + ".mp4", syn_handler, dis, config)
		cc.run()
		del cc
...
def progress(request):
	...
	try:
		progress={
			'task_name':task_name,
			'progress_rate':round(1-(get_val('_overall_view_remainder_length')/get_val('_overall_view_length')),2)}
		return HttpResponse(json.dumps(progress))
	except:
		progress={
			'task_name':task_name,
			'progress_rate':0.01
